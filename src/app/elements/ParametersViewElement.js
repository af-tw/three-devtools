...
function propsToElements(entity, elements, props, entities) {
  for (let prop of props) {
    ...
    if (prop.type === "group") {
      ...
      continue;
    } else if (prop.type === "userdata") {
      const subProps = [];
      for (const [key, value] of Object.entries(entity[prop.prop])) {
        subProps.push(html` <key-value
          uuid=${entity.uuid}
          key-name="${key}"
          .value="${value}"
          type="${prop.type}"
      ...
      let step = "step" in prop ? prop.step : type === "int" ? 1 : 0.01;
      let precision =
        "precision" in prop ? prop.precision : type === "int" ? 0 : 3;
      ...
      if (
        value &&
        entities &&
        ["geometry", "material", "texture"].indexOf(prop.type) !== -1
      ) {
        associatedData = entities[value] || {};
      }

      elements.push(html` <key-value
        uuid=${entity.uuid}
        key-name="${name}"
        .value="${value}"
        type="${type}"
...
export default class ParametersViewElement extends LitElement {
  ...
  render() {
    ...
    if (entityData) {
      ...
      const commonProps =
        entityData.type === "renderer"
          ? [CommonProps.Type, CommonProps.Name]
          : [CommonProps.Type, CommonProps.UUID, CommonProps.Name];

      propsToElements(
        entityData,
        elements,
        [...commonProps, ...definition.props],
        this.entities
      );
    }
...
<title-bar title="${entityTitle}">
  <devtools-icon-button icon="refresh" class="${
    !this.entity ? "hide" : ""
